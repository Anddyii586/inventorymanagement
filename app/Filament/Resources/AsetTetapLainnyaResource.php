...
use App\Filament\Resources\AsetTetapLainnyaResource\Pages;
use App\Filament\Resources\RelationManagers;

class AsetTetapLainnyaResource extends Resource
{
    protected static ?string $model = AsetTetapLainnya::class;

    protected static ?string $slug = 'aset-tetap-lainnya';

    protected static ?string $navigationIcon = 'heroicon-o-cube';

    protected static ?int $navigationSort = 6;

    protected static ?string $label = 'Aset Tetap Lainnya';
    ...
    public static function form(Form $form): Form
    {
        ...
        $kodeBarang = fn($set, $get, $record) => $set('id', KodifikasiService::kodeBarang($record, $get('sub_sub_kelompok_id'), $get('tanggal_pengadaan'), AsetTetapLainnya::class));
        return $form->schema([
            ...
            Forms\Components\Section::make('Kode Aset')->schema([
                Forms\Components\Select::make('sub_sub_kelompok_id')
                    ...
                    ->relationship(name: 'subSubKelompok', modifyQueryUsing: fn($query) => $query->where('id', 'like', '06.%'))
            ...
            Forms\Components\Section::make('Rincian Aset Tetap Lainnya')->schema([
                Forms\Components\TextInput::make('judul_buku')
                    ->helperText('Judul Buku/Dokumen')
                    ->maxLength(255),
                Forms\Components\TextInput::make('penulis')
                    ->helperText('Nama konsultan atau penulis dokumen')
                    ->maxLength(255),
                Forms\Components\TextInput::make('tahun_cetak')
                    ->maxLength(4),
                Forms\Components\DatePicker::make('tanggal_pengadaan'),
                Forms\Components\TextInput::make('jumlah')
                    ->helperText('Jumlah eksemplar dokumen atau buku')
                    ->numeric(),
                Forms\Components\TextInput::make('asal_usul')
                    ->helperText('Asal-usul dari buku atau dokumen. Contoh: Pembelian, hadiah dan sebagainya')
                    ->maxLength(255),
                Forms\Components\TextInput::make('harga')
                    ->helperText('Harga barang berdasarkan faktur/kuitansi pembelian apabila barang berasal dari pembelian. Apabila barang yang bersangkutan berasal dari sumbangan/hadiah supaya diperkirakan dengan harga yang wajar. Pencatatannya dalam rupiah')
                    ->prefix('Rp')
                    ->numeric(),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('penulis')
                    ->searchable(),
                Tables\Columns\TextColumn::make('tahun_cetak')
                    ->searchable(),
            ...
            ->actions([
                Tables\Actions\Action::make('qrcode')
                    ...
                    ->modalContent(fn($record) => view('filament.modals.qrcode-preview-aset-tetap-lainnya', [
                        'record' => $record,
                    ]))
    ...
    {
        return [
            'index' => Pages\ListAsetTetapLainnya::route('/'),
            'create' => Pages\CreateAsetTetapLainnya::route('/create'),
            'view' => Pages\ViewAsetTetapLainnya::route('/{record}'),
            'edit' => Pages\EditAsetTetapLainnya::route('/{record}/edit'),
        ];
    }
