...
<head>
    ...
    <title>Detail Peralatan & Mesin - {{ $peralatanMesin->id }}</title>
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-start justify-between space-y-4 sm:space-y-0">
                    <div class="flex flex-col sm:flex-row items-center sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                        ...
                        <div class="text-center sm:text-left">
                            <h1 class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-800">Detail Peralatan & Mesin</h1>
                    ...
                    <div class="text-center sm:text-right">
                        ...
                        <div class="text-lg sm:text-xl font-bold text-gray-800">{{ $peralatanMesin->id }}</div>
                ...
                <div class="xl:col-span-2 space-y-4 sm:space-y-6">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Tipe</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->tipe ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Merek</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->merek ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Bahan</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->bahan ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Kondisi</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->kondisi ?? '-' }}</p>
                            </div>
                            <div class="sm:col-span-2">
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Spesifikasi</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800" style="white-space: pre-wrap;">{!! $peralatanMesin->spesifikasi ?? '-' !!}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm sm:text-base text-gray-800 font-mono">{{ $peralatanMesin->kode_lokasi ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Wilayah</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->wilayah->wilayah ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Bidang</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->subBidang->bidang->bidang ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Sub Bidang</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->subBidang->sub_bidang ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Unit</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->unit->unit ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Ruangan</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->ruangan->nama ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->subSubKelompok()->first()->sub_sub_kelompok ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Tanggal Pengadaan</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->tanggal_pengadaan ? \Carbon\Carbon::parse($peralatanMesin->tanggal_pengadaan)->format('d/m/Y') : '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Asal Usul</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->asal_usul ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Harga</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->harga ? 'Rp ' . number_format($peralatanMesin->harga, 0, ',', '.') : '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="space-y-3 sm:space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->keterangan ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Dokumentasi</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $peralatanMesin->dokumentasi ? 'Tersedia' : 'Tidak tersedia' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="space-y-2 text-xs sm:text-sm">
                            <div>
                                ...
                                <span class="text-gray-800 block sm:inline">{{ $peralatanMesin->created_at ? \Carbon\Carbon::parse($peralatanMesin->created_at)->format('d/m/Y H:i') : '-' }}</span>
                            </div>
                            <div>
                                <span class="text-gray-600">Diubah pada:</span>
                                <span class="text-gray-800 block sm:inline">{{ $peralatanMesin->updated_at ? \Carbon\Carbon::parse($peralatanMesin->updated_at)->format('d/m/Y H:i') : '-' }}</span>
                            </div>
                            <div>
                                <span class="text-gray-600">User:</span>
                                <span class="text-gray-800 block sm:inline">{{ $peralatanMesin->user ?? '-' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
