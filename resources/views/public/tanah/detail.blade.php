...
<head>
    ...
    <title>Detail Tanah - {{ $tanah->id }}</title>
    ...
    <div class="min-h-screen py-4 sm:py-6 lg:py-8">
        <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-start justify-between space-y-4 sm:space-y-0">
                    <div class="flex flex-col sm:flex-row items-center sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                        ...
                        <div class="text-center sm:text-left">
                            <h1 class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-800">Detail Tanah</h1>
                    ...
                    <div class="text-center sm:text-right">
                        ...
                        <div class="text-lg sm:text-xl font-bold text-gray-800">{{ $tanah->id }}</div>
                        @include('public.partials.koreksi-navbar')
                    </div>
                </div>
            </div>
            ...
            @include('public.partials.asset-menu')
                    ...
                    @if(session('koreksi_success'))
                        <div class="mb-4 p-3 rounded bg-green-50 border-l-4 border-green-300 text-green-800">{{ session('koreksi_success') }}</div>
                    @endif
                ...
                <div class="xl:col-span-2 space-y-4 sm:space-y-6">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Letak/Alamat</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->letak ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Luas (m²)</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->luas ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Status Tanah</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->hak ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Nomor Sertifikat</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->nomor_sertifikat ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Tanggal Sertifikat</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->tanggal_sertifikat ? \Carbon\Carbon::parse($tanah->tanggal_sertifikat)->format('d/m/Y') : '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Penggunaan</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->penggunaan ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm sm:text-base text-gray-800 font-mono">{{ $tanah->kode_lokasi ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Wilayah</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->wilayah->wilayah ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Bidang</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->subBidang->bidang->bidang ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Sub Bidang</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->subBidang->sub_bidang ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Unit</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->unit->unit ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Tahun Penempatan</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->tahun ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->subSubKelompok()->first()->sub_sub_kelompok ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Tanggal Pengadaan</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->tanggal_pengadaan ? \Carbon\Carbon::parse($tanah->tanggal_pengadaan)->format('d/m/Y') : '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Asal Usul</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->asal_usul ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Harga</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->harga ? 'Rp ' . number_format($tanah->harga, 0, ',', '.') : '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="space-y-3 sm:space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->keterangan ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-600">Dokumentasi</label>
                                <p class="mt-1 text-sm sm:text-base text-gray-800">{{ $tanah->dokumentasi ?? '-' }}</p>
                            </div>
                        </div>
                    </div>

                    {{-- Koreksi form moved to modal partial --}}
                    @include('public.partials.koreksi-modal')
                </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                        ...
                        <div class="space-y-2 text-xs sm:text-sm">
                            <div>
                                ...
                                <span class="text-gray-800 block sm:inline">{{ $tanah->created_at ? \Carbon\Carbon::parse($tanah->created_at)->format('d/m/Y H:i') : '-' }}</span>
                            </div>
                            <div>
                                <span class="text-gray-600">Diubah pada:</span>
                                <span class="text-gray-800 block sm:inline">{{ $tanah->updated_at ? \Carbon\Carbon::parse($tanah->updated_at)->format('d/m/Y H:i') : '-' }}</span>
                            </div>
                            <div>
                                <span class="text-gray-600">User:</span>
                                <span class="text-gray-800 block sm:inline">{{ $tanah->user ?? '-' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
